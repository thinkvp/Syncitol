...
- **No ffmpeg install.** The decoder is a bundled native hybrid addon
  (~2.3 MB per platform) — a trimmed, statically linked LGPL FFmpeg 8.1.2
...
The bundled FFmpeg decoder is a UXP *hybrid* addon, which Premiere only loads
from 26.0 onward — on 25.x the panel opens but the decoder reports "Addon is
not supported", so 26.0 is the real floor.
...
3. On Windows, you may be prompted for your account credentials as part of
   ...
   is a normal elevation prompt, not a malware warning.
...
> **macOS: expect a security warning the first time.** After installing, run
> Premiere Pro and it'll say it can't load the plugin. Go into **System
> Settings → Privacy & Security**, click **Allow Anyway** on the message about
> Syncitol, then restart Premiere Pro. The only way I can get rid of that
> warning is by signing up to a $99/year Apple Developer account. I don't use
> Mac and this is a free plugin, so it's not something I'm interested in
> doing.
...
The plugin source lives in [`uxp/`](uxp/); all commands below run from there.
...
- **Load:** UXP Developer Tool, or the CLI: `uxp service start`, then
  `uxp plugin load|reload --apps premierepro` from `uxp/`.
...
The addon source lives under `uxp/native/`. Build scripts:
...
[MIT](LICENSE) for Syncitol's own code. Bundled third-party components keep
...
under the OFL (`uxp/fonts/LICENSE.txt`). If distributed commercially, ship an
